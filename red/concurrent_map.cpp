...
public:
  ...
  struct Access {
    ...
    Access  (V& value, lock_guard<mutex> &g) : ref_to_value(value), guard(g) {}
  };

  explicit ConcurrentMap(size_t count)
    : Map(count),
      q(count),
      size(count){
    }
  ...
  Access operator[](const K& key){
    auto key1 = Abs(key);
    return {Map[key1%size][key1], lock_guard(q[key1%size])};
  ...
  map<K, V> BuildOrdinaryMap() {
    ...
    //for (auto &j: Map) {
        auto it = lock_guard {q[i]};
        result.insert(begin(Map[i]),end(Map[i]));

    }
    return result;
  }
private:
    //vector<pair<mutex, <map<K, V>> IO;
    vector <map<K, V>> Map;
    vector <mutex> q;
    size_t size;
};
