...
class AirportCounter {
    public:
        ...
        size_t Get(TAirport airport) const {
            return data[static_cast<uint64_t>(airport) ];
        }

        // добавить данный элемент
        void Insert(TAirport airport){
            data[static_cast<uint64_t>(airport)]++;
        }

        // удалить одно вхождение данного элемента
        void EraseOne(TAirport airport){
            if (data[static_cast<uint64_t>(airport)] > 0) {
                data[static_cast<uint64_t>(airport)]--;
            }
        }
        ...
        void EraseAll(TAirport airport){
            data[static_cast<uint64_t>(airport)] = 0;
        }
        ...
        Items GetItems() const {
            ...
            for(size_t i=0; i<SIZE; i++) {
                PP[i] = make_pair( static_cast<TAirport>(i), data[i] );
            }
            return PP;
        }
       //sdsadasdsadsa
//asdsaaadsadas
    private:
        array <size_t, static_cast<uint64_t>(TAirport::Last_)> data;
